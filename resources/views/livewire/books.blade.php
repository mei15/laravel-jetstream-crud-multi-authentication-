...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
            ...
            @endif
            @if( Auth::user()->userable_type == 'App\SuperAdmin')
            <button wire:click="create()" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded my-3">Tambah Buku</button>
            @endif
            @if($isModal)
                @include('livewire.create')
            @endif

            <table class="table-fixed w-full center">
                <thead>
                ...
                <tbody>
                    @forelse($books as $row)
                        <tr>
                            ...
                            <td class="border px-4 py-2 text-center">{!! $row->status_label !!}</td>
